"""DOCSTRING: Flask-based Othello game engine with AI opponent."""
...
def move():
    ...
    if not check_if_any_legal(board, current_player):
        ...
        if not check_if_any_legal(board, current_player):
            return finish_game(board, "No More Legal Moves!")
        session["current_player"] = current_player
        return jsonify({"status": "fail", "message": msg})

    legal, end_points_list = legal_move(current_player, (x, y), board, output=True)
    if not legal:
        return jsonify({"status": "fail", "message": f"Illegal move at ({x},{y})."})

    play_move(board, (x, y), current_player, end_points_list)
    move_count -= 1
    current_player = switch_player(current_player)

    ai_colour = session["ai_colour"]
    if current_player == ai_colour and check_if_any_legal(board, ai_colour):
        best_move = get_best_move(board, ai_colour, depth=2, ai_col=ai_colour)[0]
        if best_move:
            legal, end_points_list = legal_move(ai_colour, best_move, board)
            play_move(board, best_move, ai_colour, end_points_list)
            move_count -= 1
            current_player = switch_player(current_player)
    ...
    if move_count <= 0:
        return finish_game(board, "Out Of Turns!")
...
def finish_game(board, reason):
    score_dict = count_tiles(board)
    dark_tiles, light_tiles = score_dict['Dark'], score_dict['Light']
